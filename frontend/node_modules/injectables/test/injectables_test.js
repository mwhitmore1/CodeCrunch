...
describe('Injectables', function(){
  ...
  it('Should be able to load multiple dependencies in a single call', function(){
    var mdls = injector.get(['::injectr/injectr.js', ['::mocha::interfaces.bdd', 'mocha'] ]);
    expect(mdls).to.be.Array;
    expect(mdls).to.have.length(2);
    expect(mdls[0]).to.not.be.null;
    expect(mdls[1]).to.not.be.null;
  });
